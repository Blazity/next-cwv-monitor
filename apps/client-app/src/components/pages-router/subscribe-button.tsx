...
import { useTrackCustomEvent } from 'next-cwv-monitor/pages-router';
...
export default function SubscribeButton({ onClick, ...props }: Props) {
  ...
  return (
    <BaseCustomEventButton
      ...
      className={
        'cursor-pointer rounded-xl bg-black px-4 py-2 text-white transition-colors hover:bg-zinc-800 disabled:cursor-not-allowed'
      }
      onClick={handleClick}
    />
  );
