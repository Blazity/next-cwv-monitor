...
export function ManageTab({ eventsDisplaySettings: settings, eventNames, projectId }: Props) {
  ...
  const handleToggleVisibility = (eventName: string) => {
    setEventDisplaySettings((prev) => {
      const prevEventSettings = prev?.[eventName];
      const isHidden = prevEventSettings === undefined ? true : !prevEventSettings.isHidden;

      return { ...prev, [eventName]: { ...prevEventSettings, isHidden } };
    });
  };
              ...
              {filteredEvents.map((event) => {
                ...
                return (
                  <div
                    key={event}
                    className={cn(
                      `border-border bg-background flex items-center justify-between rounded-lg border p-4`,
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            type="button"
                            className={cn(
                              `text-status-good hover:bg-status-good/10 rounded-md p-1.5 transition-colors`,
                        ...
                        {editedEvent === event ? (
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-sm font-medium ${eventSettings?.isHidden ? 'text-muted-foreground' : 'text-foreground'}`}
                              >
                                {eventSettings?.customName || event}
                              </span>
                            </div>
