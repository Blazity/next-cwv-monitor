...
describe('daily-aggregates-repository', () => {
  ...
  describe('getProjectMetricOverview', () => {
    ...
    it('returns aggregated metrics from materialized view', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'agg-test', name: 'Aggregation Test' });
    ...
    it('returns multiple metrics when they exist', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'multi-metric', name: 'Multi Metric' });
    ...
    it('filters aggregates by device type', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'device-filter', name: 'Device Filter' });
    ...
    it('returns quantile summary with all percentiles', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'quantiles-test', name: 'Quantiles Test' });
  ...
  describe('getRouteDailySeries', () => {
    it('returns daily series for a specific route and metric', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'daily-series', name: 'Daily Series' });
    ...
    it('returns empty array for non-existent route', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'no-route', name: 'No Route' });
    ...
    it('filters daily series by device type', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'series-device', name: 'Series Device' });
    ...
    it('returns series ordered by date ascending', async () => {
      const projectId = randomUUID();
      await createProject({ id: projectId, slug: 'date-order', name: 'Date Order' });
